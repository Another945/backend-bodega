...
const conexion = mysql.createConnection({
    host: 'localhost',
    user: 'root',
    password: '933638746Doro',
    database: 'bodega_nanf'
});
...
app.get('/merma', (req, res) => {
    conexion.query(
        'SELECT * FROM merma ORDER BY id DESC',
        (error, resultados) => {
            if (error) return res.status(500).json(error);
            res.json(resultados);
        }
...
app.get('/reportes', (req, res) => {
    ...
    conexion.query(
        'SELECT IFNULL(SUM(total),0) AS ventasHoy FROM ventas WHERE DATE(fecha) = CURDATE()',
        (err, ventasHoy) => {
            if (err) return res.status(500).json(err);
            datos.ventasHoy = ventasHoy[0].ventasHoy;

            conexion.query(
                'SELECT IFNULL(SUM(total),0) AS ventasTotales FROM ventas',
                (err, ventasTotales) => {
                    if (err) return res.status(500).json(err);
                    ...
                    conexion.query(
                        ...
                        (err, productos) => {
                            ...
                            conexion.query(
                                'SELECT COUNT(*) AS stockBajo FROM productos WHERE stock <= 10',
                                (err, stock) => {
                                    if (err) return res.status(500).json(err);
                                    datos.stockBajo = stock[0].stockBajo;

                                    conexion.query(
                                        'SELECT IFNULL(SUM(perdida),0) AS mermaTotal FROM merma',
                                        (err, merma) => {
                                            if (err) return res.status(500).json(err);
                                            datos.mermaTotal = merma[0].mermaTotal;
        ...
        (err, ventasHoy) => {
            ...
            conexion.query(
                ...
                (err, ventasTotales) => {
                    ...
                    conexion.query(
                        'SELECT id, fecha, total FROM ventas ORDER BY fecha DESC LIMIT 10',
                        (err, historialVentas) => {
                            if (err) return res.status(500).json(err);
